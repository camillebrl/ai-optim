...
normalize_forimagenet = transforms.Normalize(mean=[0.485, 0.456, 0.406],std=[0.229, 0.224, 0.225]) # Ce ne sont pas les mêmes datasets donc pas les mêmes moyennes, etc.
...
transform_train_imagenet = transforms.Compose([
    ...
    transforms.ToTensor(),
    normalize_forimagenet, # Dans imagenet, il y a des moyennes et ecart-types differents pour chaque channel (rouge, bleu, vert)
])
