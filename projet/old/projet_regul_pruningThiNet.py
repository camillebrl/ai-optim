...
def count_avgpool(m, x, y):
    x = x[0]
    total_add = torch.prod(torch.Tensor([m.kernel_size]) - 1,device="cuda:0")
    total_div = torch.Tensor(1,device="cuda:0")
    kernel_ops = total_add + total_div
    num_elements = torch.Tensor(y.numel(),device="cuda:0")
...
def profile(model, input_size, custom_ops = {}):
    ...
    def add_hooks(m):
        ...
        for p in m.parameters():
            m.total_params += torch.Tensor([p.numel()],device="cuda:0") / 2  # Division Free quantification
