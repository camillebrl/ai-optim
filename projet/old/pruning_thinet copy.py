            ...
            if isinstance(m, Bottleneck):
                ...
                for m_2 in m.modules():
                    if isinstance(m_2, nn.Conv2d):
                        convs.append(m_2)
                # we take all conv of each block except the last one
    ...
    def thinet(self, trainloader, p_to_delete):
        ...
        for mod, m in enumerate(self.target_modules):
            ...
            if isinstance(m, nn.Conv2d):
                list_training = []
                n = 64
                # récupère au hasard les indices de n (=64) batch dans trainloader
                ...
                for i, (inputs, targets) in enumerate(trainloader):
                    ...
                    if i in subset_indices:
                        for j in range(inputs.size()[0]):
                            ...
                            output=m._input_hook[0] # on supprime la première dimension qu'on a ajouté précédemment 
                ...
                while len(channels_to_delete) < c * p_to_delete:
                    ...
                    for channel in total_channels:
                        ...
                        a_changed = batch[:,channels_to_try_to_delete, :, :] # plus rapide pour pytorch de sommer un batch que de sommer une liste
                        result = torch.sum(a_changed,dim=(1,2,3)) # On ne somme pas sur la 1ère dimension 0
            ...
            if isinstance(m, nn.Conv2d):
                ...
                while len(channels_to_delete) < c * p_to_delete:
                    ...
                    for channel in total_channels:
                        ...
                        batch=torch.stack(list_training)
                        a_changed = batch[:,:,channels_to_try_to_delete, :, :] # plus rapide pour pytorch de sommer un batch que de sommer une liste
                        result = torch.sum(a_changed,dim=(2,3,4)) # On ne somme pas sur la 1ère dimension 0
                        value = torch.sum(result**2)
                        if value < min_value:
                            min_value = value
                            min_channel = channel
