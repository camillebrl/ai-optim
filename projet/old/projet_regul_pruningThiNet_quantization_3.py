...
def function_separation_on_tensor(nb_bits, tensor, device):
    ...
    return results.view(
        tensor.size())  # gather a besoin d'avoir les mêmes dimensions pour les 2
...
def hook(module, input, output):
    if hasattr(module, "_input_hook"):
        module._input_hook = input[
            0]  # input contient plusieurs inputs différents? ça ne fonctionne sans [0] car ça renvoie un tuple...
            ...
            if isinstance(m, Bottleneck):
                ...
                for m_2 in m.modules():
                    if isinstance(m_2, nn.Conv2d):
                        convs.append(m_2)
                self.target_modules.extend(convs[
                                           :-1])  # on prend toutes les conv2d de chaque block sauf la dernière
        ...
        for m in self.target_modules:
            if isinstance(m, nn.Conv2d):
                m.register_forward_hook(
                    hook)  # register_forward_hook prend un objet fonction en paramètre
        for mod, m in enumerate(
                self.target_modules):  # Dans le papier, il est indiqué que 90% des floating points operattions sont contenus dans les 10 premiers layers
            print("module:", mod)
            if isinstance(m, nn.Conv2d):
                list_training = []
                n = 64
