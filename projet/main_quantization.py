...
model_pruned=torch.load(f"{dataset}/models/half_ResNet18_divParamOf_1_functionOf_simple_regCoefOf_0.1_ThiNet_pruning_retrain_rate_0.2_learningRateOf_0.001_momentumOf_0.95_weightDecayOf_5e-05_gradDescentMethodOf_SGD_schedMethodOf_CosineAnnealingLR.pt")
quantization_hyperpara=Quantization_Hyperparameters(learning_rate,weight_decay,momentum,loss_function,gradient_method,nb_bits)
optimizer = optim.SGD(model_pruned.model.parameters(), lr=learning_rate,momentum=momentum, weight_decay=weight_decay)
scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer,T_max=n_epochs)

def quantize_nbits(model_pruned,trainloader,validloader,quantization_hyperpara,optimizer,scheduler,device):
    fname=quantization_hyperpara.build_name()
    base_dir = f"./{dataset}/"
    model_dir = os.path.join(base_dir, "models")
    results_dir = os.path.join(base_dir, "results")
    fname_model = model_dir+fname+".pt"
    fname_results = results_dir+fname + ".csv"
    print("Saving model"+ fname_pruning_retrain_half_bc_model)
    print("Saving results in :", fname_pruning_retrain_half_bc_results)

    bc_model = BC(model_pruned.model, nb_bits, device)
