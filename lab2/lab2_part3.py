...
def measuretime(f,args=None):
    #mesure le temps d'exécution d'une fonction
    start=dt.datetime.now()
    r=f(*args) if args is not None else f()
    end=dt.datetime.now()
    return r,end-start
...
class BC():
    def __init__(self, model,nb_bits):
    ...
    def binarization(self):
        ...
        for index in range(self.num_of_params):
            self.target_modules[index].data.copy_(function_separation_on_tensor2(self.nb_bits,self.target_modules[index].data))
...
def models_variant_archi_param(n_epochs=200,learning_rate=0.001,momentum=0.95,weight_decay=5e-5,method_gradient_descent="SGD",method_scheduler="CosineAnnealingLR",loss_function=nn.CrossEntropyLoss()):
    for model_id in cfg_chosen.keys():
        ...
        for nb_bits in nb_bits_list:
            ...
            file_name=f"{model_id}_{nb_bits}_{learning_rate}_{momentum}_{weight_decay}_{method_gradient_descent}_{method_scheduler}.csv"
            results=train_model(model,device,loss_function,n_epochs,trainloader,validloader,scheduler,optimizer)
            results.to_csv("./models_dif_precision_with_dif_nb_param/"+str(count_parameters(model.model))+"_"+file_name)
