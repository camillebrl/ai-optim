...
class Pruning():
    ...
    def thinet(self,p_to_delete):
        ...
        for m in self.target_modules:
            if isinstance(m, nn.Conv2d):    
                list_training=[]
                for i in range(len(m._inputs_hook)): # on itère sur le nombre de batchs (il y a 100 batchs ici, on a bien len(m._inputs_hook)=100)
                    for j in range(len(m._inputs_hook[i])):
                        n=3  # mettre ene paramètre
                    # Je n'ai pas bien compris dans le papier combien de couple x,y on devait prendre... 
                    # Là, j'en prends le même nombre que le nb de batch, or je devrais prendre nombre d'images (32*100) * nombre de locations non? C'est quoi le nb de locations?
                        ...
                        x=m._inputs_hook[i][j,:,ligne:ligne+w.size()[1],colonne:colonne+w.size()[2]]
                        target = torch.zeros(w.size()[0], m._inputs_hook[i].size()[2]+2, m._inputs_hook[i].size()[3]+2) #w.size()[0] #channels d'entrée, m._inputs_hook[i].size()[2]: taille entrée en largeur, m._inputs_hook[i].size()[3]+2: taille de l'entrée en hauteur.
                        target[:, 1:m._inputs_hook[i].size()[2], 1:m._inputs_hook[i].size()[3]] = x
