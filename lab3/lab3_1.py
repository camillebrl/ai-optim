...
class Pruning():
    ...
    def thinet(self,p_to_delete):
        ...
        for mod,m in enumerate(self.target_modules[:7]): # Dans le papier, il est indiqué que 90% des floating points operattions sont contenus dans les 10 premiers layers
            print("module: ",mod)
            if isinstance(m, nn.Conv2d):    
                list_training=[]
...
def models_variant_archi_param(n_epochs=2,learning_rate=0.001,momentum=0.95,weight_decay=5e-5,method_gradient_descent="SGD",method_scheduler="CosineAnnealingLR",loss_function=nn.CrossEntropyLoss()):
    for model_id in cfg_chosen.keys():
        ...
        for rate in rate_to_delete:
            ...
            resultats=train_model(model_pruning.model,device,loss_function,n_epochs,trainloader,validloader,full_trainloader,testloader,scheduler,optimizer)
            resultats.to_csv("./thinet_dif_rate_with_dif_nb_param/"+str(nb_parameter_in_vgg)+str(rate)+"thinet.csv")
