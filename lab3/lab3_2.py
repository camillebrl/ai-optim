...
class Pruning():
    ...
    def thinet(self,p_to_delete,nb_of_layer_to_prune=10):
        ...
        test(0,self.bc_class,device="cuda:0", testloader=trainloader) # on doit faire un test car sinon la fonction register_forward_hook n'a pas accès au forward
        ...
        for mod,m in enumerate(self.target_modules[:nb_of_layer_to_prune]): # Dans le papier, il est indiqué que 90% des floating points operattions sont contenus dans les 10 premiers layers
            ...
            if isinstance(m, nn.Conv2d):    
                ...
                #m._inputs_hook[i]=m._inputs_hook[i][total_channels,:,:]
...
def models_variant_archi_param(n_epochs=200,learning_rate=0.001,momentum=0.95,weight_decay=5e-5,method_gradient_descent="SGD",method_scheduler="CosineAnnealingLR",loss_function=nn.CrossEntropyLoss()):
    for model_id in cfg_chosen.keys():
        ...
        for rate in rate_to_delete:
            ...
            torch.cuda.empty_cache()
            resultats=train_model(model_pruning.bc_class,device,loss_function,n_epochs,trainloader,validloader,scheduler,optimizer)
            torch.cuda.empty_cache()
            resultats.to_csv("./binarized_thinet_dif_rate_with_dif_nb_param/"+"thinet"+str(rate)+str(count_parameter)+file_name)
